...
// Запись на урок с поддержкой кнопок
func handleEnrollCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message, db *sql.DB) {
	// Показываем доступные уроки с кнопками для записи
	rows, err := db.Query(`
		...
		WHERE l.start_time > NOW() AND l.start_time < NOW() + INTERVAL '3 days'
	...
	if err != nil {
		sendMessage(bot, message.Chat.ID, "❌ Ошибка загрузки доступных уроков")
		return
	}
	defer rows.Close()
	...
	if !hasLessons {
		sendMessage(bot, message.Chat.ID, "📝 На ближайшие дни нет доступных уроков для записи")
	}
}
